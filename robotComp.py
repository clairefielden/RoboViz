...
class RobotComp:
    ...
    def __init__(self, id, type, root, orientation):
        ...
        self.dst_pos = 0
        self.deltaX = 0
        #save the destination position to be changed with robot stepping in ann
    ...
    def calcAccelaration(self):
        ...
        Returns: The accelaration (double)
        """
        a = self.mass*9.8
        return a
    ...
    def calcPos(self, src, dst, connection):
        ...
        if src_slot == 0:                                               # use src slot to determine which side to place dest model
            self.dst_pos = src_pos + LVector3f(0, -(src_dim + dst_dim), 0)
        elif src_slot == 1:
            self.dst_pos = src_pos + LVector3f(-(src_dim + dst_dim), 0, 0)
        elif src_slot == 2:
            self.dst_pos = src_pos + LVector3f(0, src_dim + dst_dim, 0)
        elif src_slot == 3:
            self.dst_pos = src_pos + LVector3f(src_dim + dst_dim, 0, 0)
        return (self.dst_pos, heading)
