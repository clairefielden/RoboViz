...
class RobotGUI:
    ...
    def __init__(self, config_path='', pos_path='', robot_path='', cli=False):
        ...
        Args:
            ...
            'cli': whether or not the command line interface is being used or not (Boolean)
            'ANN': whether or not the brain shoulld be created or not (Boolean)
        """
    ...
    def startGUI(self):
        ...
        while True:
            ...
            if ((event == "Submit" and values["-FILE_PATH-"] != "" and values["-FILE_PATH-0"] != "" and values["-FILE_PATH-2"] != "") or (event == 'Submit' and values['-A_PACK-'])):
                ...
                self.utils = RobotUtils(self.config_path, self.pos_path, self.robot_path)
                config, self.ANN = self.utils.configParse()
                if not config:
    ...
    def runSim(self, config='', robots='', auto_pack=False):
        ...
        if self.cli:
            ...
            robots = self.utils.robotParse(int(config[2]), positions)
            #if the config file has only 1 robot, create the neural network
            if self.ANN==True:
                neuralNet = self.utils.createBrain()
        ...
        for i, robot in enumerate(robots):                                      # loop through robots in swarm
            ...
            if self.ANN == True:
                env.stepNetwork(neuralNet, robot) #if ANN = true, then step the network
